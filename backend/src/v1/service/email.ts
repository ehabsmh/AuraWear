...
import { APP_DOMAIN, APP_PORT } from "../..";
...
export default async function sendVerificationCode(newUser: any) {
  const info = await transporter.sendMail({
    from: "Aura Wear 👕 <ehabsmh3@gmail.com>",
    to: newUser.email,
          ...
          <body>
            <div class="c-email">  
              ...
              <p class="c-email__content__text text-italic opacity-30 text-title mb-0">
                Verification code is valid only for 15 minutes
              </p>
              <p class="c-email__content__text text-italic opacity-30 text-title mb-0">
                If you lost the verification page, click the link below to verify your account:

              </p>
              <p class="c-email__content__text text-italic opacity-30 text-title mb-0">
                <a href="${APP_DOMAIN}:${APP_PORT}/verify?email=${newUser.email}" target="_blank">
                  Verify Account
                </a>
              </p>
              </div>
